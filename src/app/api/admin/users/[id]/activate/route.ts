...
export async function POST(
  ...
  try {
    ...
    await logAdminActivity(
      ...
      `Re-approved / activated user account "${updatedProfile.username}"`,
      "user",
      userId,
      { action: "USER_ACTIVATED", activated_at: now }
    );

    return NextResponse.json({
      success: true,
      profile: updatedProfile,
      message: `User ${updatedProfile.username} has been activated.`,
    });
  } catch (err: any) {
    return NextResponse.json(
      { error: err?.message || "Failed to activate user account." },
      { status: 500 }
    );
  }
}
