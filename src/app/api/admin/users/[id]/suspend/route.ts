...
export async function POST(
  ...
  try {
    ...
    await logAdminActivity(
      ...
      `Suspended user account "${updatedProfile.username}"`,
      "user",
      userId,
      { action: "USER_SUSPENDED", suspended_at: now }
    );

    return NextResponse.json({
      success: true,
      profile: updatedProfile,
      message: `User ${updatedProfile.username} has been suspended.`,
    });
  } catch (err: any) {
    return NextResponse.json(
      { error: err?.message || "Failed to suspend user account." },
      { status: 500 }
    );
  }
}
